...
class EthAddressListAdapter(
    ...
    fun changeAddress(position: Int, holder: AddressItemVH) {
        currentAcc.changeEthByBip44Index(listIndex[position])?.let { newAddr ->
    ...
    override fun onBindViewHolder(
        holder: AddressItemVH,
        position: Int,
        payloads: MutableList<Any>
    ) {
        currentAcc.getEthAddressByBip44Index(listIndex[position])?.let { addr ->
            if (payloads.isEmpty()) {
                holder.setAddress(addr, position)
            }
            if (addr == currentAcc.nowEthAddress()) {
                holder.changeRadioButton.isChecked = true
                selectedIndex = position
            } else {
