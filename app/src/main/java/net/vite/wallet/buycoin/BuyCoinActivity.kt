...
class BuyCoinActivity : BaseTxSendActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        coinPurchaseViewModel.exchangeLd.observe(this, Observer {
            ...
            if (it?.resp?.code == 0) {
                BuyCoinRecordActivity.show(currentAccount.nowViteAddress()!!, this)
            } else {
                showToast(getString(R.string.bugcoin_failed, it?.resp?.code?.toString()))
            }
        })
    ...
    private fun refreshConvertRate(it: ConvertRateResp) {
        ethVitePrice.text =
            getString(
                R.string.buy_coin_price,
                it.rightRate?.toBigDecimal()?.toLocalReadableText(8) ?: ""
            )
        oneDayLimit.text = getString(
            ...
            it.quota?.quotaRest?.toBigDecimal()?.toBigInteger()?.toBigDecimal()
                ?.toLocalReadableTextWithThouands() ?: ""
        )
    ...
    private fun buy() {
        ...
        val viteEthTokenInfo =
            TokenInfoCenter.getTokenInfoInCache(TokenInfoCenter.ViteEthTokenCodes.tokenCode)
                ?: return
    ...
    private fun setupAmountInputEditText() {
        ...
        val viteViteTokenInfo =
            TokenInfoCenter.getTokenInfoInCache(TokenInfoCenter.ViteViteTokenCodes.tokenCode)
                ?: return
        viteCoinIcon.setup(
