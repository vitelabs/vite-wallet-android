...
import net.vite.wallet.network.rpc.GetPledgeQuotaResp
import net.vite.wallet.network.rpc.SyncNet
import java.util.concurrent.ConcurrentHashMap
import java.util.concurrent.TimeUnit

object QuotaAndTxNumPoll {
    var disposable: Disposable? = null
    val latestData = ConcurrentHashMap<String, GetPledgeQuotaResp>()
    fun getLatestData(viteAddress: String): GetPledgeQuotaResp? {
        return latestData[viteAddress]
    }

    private var id = 0
    private val liveDataSet = HashMap<Int, MutableLiveData<GetPledgeQuotaResp>>()

    fun register(liveData: MutableLiveData<GetPledgeQuotaResp>): Int {
        id++
    ...
    fun start() {
        disposable = Observable.fromCallable {
            SyncNet.getPledgeQuota(AccountCenter.currentViteAddress())
