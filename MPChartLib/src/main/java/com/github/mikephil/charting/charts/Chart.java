...
public abstract class Chart<T extends ChartData<? extends IDataSet<? extends Entry>>> extends
    ...
    protected boolean logoEnable = false;
    ...
    protected void init() {
        ...
        logoBitmap = BitmapFactory.decodeResource(getResources(), R.mipmap.kline_logo);
...
//            position.y += Utils.calcTextHeight(mDescPaint, mDescription.toString());
            ...
            if (mIsDescriptionCustom) {
                ...
                if (position == null) {
                    for (int i = 0; i < mDescriptionLabels.length; i++) {
                        ...
                        float y = mViewPortHandler.contentTop() + 20 + Utils.calcTextHeight(mDescPaint, mDescription.toString()) / 2;

                        c.drawText(mDescriptionLabels[i], xpos, y, mDescPaint);
                    }
...
//            c.drawText(mDescription.getText(), x, y, mDescPaint);
            if (logoEnable && logoBitmap != null) {
                float y = mViewPortHandler.contentBottom() - 20 - logoBitmap.getHeight();
                c.drawBitmap(logoBitmap, mViewPortHandler.contentLeft(), y, null);
            }
        }
    }
